...
- `ai_figure_specs` 建议为每张图补齐 `diagram_type` 与 `style_notes`，让提示词更接近论文技术图而不是泛化插画
...
- `version` 会输出当前 bundle 的 semver 版本、对应 tag、当前 commit、dirty 状态和最近 semver tag，便于做正式发版
