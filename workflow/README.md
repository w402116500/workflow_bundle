...
10. 如需 AI 场景插图且依赖教程图/样图，先执行 `python3 workflow_bundle/tools/cli.py prepare-reference-guides --config <workspace-dir>/workflow/configs/workspace.json`
11. 再执行 `python3 workflow_bundle/tools/cli.py prepare-ai-figures --config <workspace-dir>/workflow/configs/workspace.json`
...
- `prepare-reference-guides` 会读取 `reference_extraction` 与 `reference_guide_specs`，把教程图/参考图/说明文档先抽成 `docs/images/reference_guides/*.json|*.md`
...
- `prepare-ai-figures` 不应继续只靠原始 `reference_images` 猜图法；如有教程图或样图，优先先抽成 `reference_guides`，再由图号消费 guide，`reference_images` 退化为辅助视觉参考
- `prepare-ai-figures` 不应继续使用泛化“学术插画”提示词；建议在 `ai_figure_specs` 中显式补齐 `diagram_type`、`style_notes` 与 `reference_guides`，把目标收敛到白底、黑线、二维、论文技术图风格
...
- 如果图号声明了 `reference_guides`，但 guide JSON 缺失或过期，则 `prepare-ai-figures` 与 `release-preflight` 会直接失败，要求先重新执行 `prepare-reference-guides`
...
- `VERSION` 是当前 bundle 的正式版本单一真源；`python3 workflow_bundle/tools/cli.py version` 可查询版本号、建议 tag、tag 是否已存在、当前 commit 和 dirty 状态
- `prepare-figures` 额外支持 `plantuml_figure_specs`：可按图号显式声明某张流程图、用例图或其他 UML 图走本地 `PlantUML` 源驱动渲染，而不是继续依赖临时位图
- `plantuml_figure_specs` 的正式输入是 workspace config；启用后工作流会在 `docs/images/generated_src/` 额外写出 `.puml/.svg` 侧车文件，并把最终 PNG 写入 `figure_map`
- `plantuml` renderer 使用 bundle 内 vendored PlantUML jar，并自动寻找 Java 11+ 运行时；建议在 `.puml` 中显式声明 `!pragma layout smetana`，以减少外部 Graphviz 依赖
