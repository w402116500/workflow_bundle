...
def _generate_agent_task(paper: dict[str, Any], output_md: Path, reader_script_rel: str, standards_rel: str) -> str:
...
- PDF Path: {paper.get('pdf_path', '')}
- Output Report: {output_md}
- Analysis Standard: {standards_rel}

Step 1: Read `{standards_rel}`.
Step 2: Extract the paper:
```bash
python3 {reader_script_rel} "{paper.get('pdf_path', '')}" --full --output-dir "{extracted_dir}"
```
Step 3: Write a 6-section analysis report to `{output_md}` without fabricating data.
"""


def _write_agent_tasks(downloaded_papers: list[dict[str, Any]], research_dir: Path, reader_script_rel: str, standards_rel: str) -> list[dict[str, Any]]:
    ...
    _write_json(research_dir / "_agent_tasks.json", tasks)
    return tasks
...
def run_research_sidecar(queries: list[str], research_dir: Path, max_papers: int, reader_script_rel: str, standards_rel: str) -> dict[str, Any]:
    ...
    status = "analyses_present" if analysis_count > 0 else ("prepared" if papers else "failed")
    summary = {
        ...
        "errors": errors,
        "papers": _summarize_papers(research_dir, papers),
    }
    _write_json(research_dir / "_research_summary.json", summary)
    return summary


def build_research_index(summary: dict[str, Any], research_dir: Path, research_index_json: Path, research_index_md: Path) -> dict[str, Any]:
    index = {
        "generated_at": _now_iso(),
    ...
    _write_json(research_index_json, index)
    _write_text(research_index_md, render_research_index_md(index))
    return index
